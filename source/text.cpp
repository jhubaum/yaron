...
static ShaderPtr shader = nullptr;
...
Text::Text(const std::string &text, const glm::vec2 &pos, const Color &col, float scale)
...
TextRenderer::TextRenderer(const std::map<char, Character> &chars, GLuint vao, GLuint vbo)
...
void TextRenderer::renderText(GLFWwindow *window, const Text &text) {
  int width, height;
  ...
  shader->setActive();
  shader->enableBlending(BlendFunc::SrcAlpha, BlendFunc::OneMinusSrcAlpha);
